...
def create_dataframe_length_group(filename1, filename2, col_name1, col_name2):
    ...
    df = df[df.apply(lambda x: 20 <= len(x[col_name1].split(' ')) < 30, axis=1)]
...
zh_en_val = create_dataframe_length_group('./iwslt-zh-en/dev.tok.zh', './iwslt-zh-en/dev.tok.en', 'zh', 'en')
...
max_vocab_size_zh_en_en = 10000
max_vocab_size_zh_en_zh = 10000
...
def evaluate_BLEU(encoder, decoder, sentence, target_id2token, max_length=MAX_LENGTH):
    """
    ...
    for j in range(max_length):
        ...
        #print(topi.shape)
        word = target_id2token[topi.item()]
...
def compute_bleu(encoder, decoder, test_loader, source_id2token, target_id2token, max_length = MAX_LENGTH):
    source_stream = []
    target_stream = []
    decoded_stream = []
    ...
    for idx, (source_sents, target_sents, source_lengths, target_lengths) in enumerate(test_loader):
        ...
        for i in range(len(source_sents)):
            ...
            decoded_words, decoder_attentions = evaluate_BLEU(encoder, decoder, sentence, target_id2token, max_length)
            target_words = []
            source_words = []
            ...
            for j in range(len(target_sents[i])):
                if target_sents[i][j] == EOS_IDX: 
                    break
                target_words.append(target_id2token[target_sents[i][j]])
            source_sentence = ' '.join(source_words)
            target_sentence = ' '.join(target_words)
            output_sentence = ' '.join(decoded_words[: -1])
            
            source_stream.append(source_sentence)
            target_stream.append(target_sentence)
            decoded_stream.append(output_sentence)

    return sacrebleu.corpus_bleu(target_stream, [decoded_stream]), [source_stream[-1]], [decoded_stream[-1]], [target_stream[-1]], decoder_attentions


# From lab     
def showAttention(input_sentence, output_words, attentions, idx, lang):
    # Set up figure with colorbar
    fig = plt.figure()
    ax = fig.add_subplot(111)
    attentions_new = np.array([attention.view(-1).numpy() for attention in attentions])
    cax = ax.matshow(attentions_new, cmap='bone')
    fig.colorbar(cax)

    # Set up axes
    ax.set_xticklabels([''] + input_sentence[0].split(' '), rotation=90)
    ax.set_yticklabels([''] + output_words[0].split(' ') + ['<EOS>'])

    print(len(input_sentence[0].split(' ')))
    print(len(output_words[0].split(' ')) + 1)
    print(attentions_new.shape)
    # Show label at every tick
    ax.xaxis.set_major_locator(ticker.MultipleLocator(1))
    ax.yaxis.set_major_locator(ticker.MultipleLocator(1))

    plt.savefig( lang+'_attention_test_idx_'+str(idx))


def evaluateAndShowAttention(input_sentence_idx, encoder, decoder, test_loader, language_pair, source_id2token, target_id2token):
    for i, (source_sents, target_sents, source_lengths, target_lengths) in enumerate(test_loader):
        if i == input_sentence_idx:
            curr_test_loader = [(source_sents, target_sents, source_lengths, target_lengths)]
    _, input_sentence, output_words, target_sentence, attentions = compute_bleu(encoder, decoder, curr_test_loader, source_id2token, target_id2token)
    print(input_sentence)
    print(target_sentence)
    print(output_words)


    showAttention(input_sentence, output_words, attentions, input_sentence_idx, language_pair)



def compute_bleu_and_show_attention(lang_pair, epoch_no, source_id2token, target_id2token, test_loader):

    epoch = epoch_no
    lang_pair = lang_pair

    encoder = EncoderRNN(len(source_id2token), 300, 1, bidirectional=True).to(device)
    decoder = AttnDecoderRNN(300, len(target_id2token), attention_mechanism = 'dot').to(device)

    encoder.load_state_dict(torch.load("attn_encoder_batched_" + lang_pair + '_epoch_' + str(epoch)+ ".pth", map_location='cpu'))
    decoder.load_state_dict(torch.load("attn_decoder_batched_" + lang_pair+ '_epoch_' + str(epoch) + ".pth", map_location='cpu'))

    bleu_score, _, _, _, _ = compute_bleu(encoder, decoder, test_loader, source_id2token, target_id2token)
    print(bleu_score)

    # for i in range(1,100):
    #     evaluateAndShowAttention(i, encoder, decoder, test_loader, lang_pair, source_id2token, target_id2token)
    


compute_bleu_and_show_attention('zh_en', 2, zh_to_en_zh_id2token, zh_to_en_en_id2token, zh_to_en_val_loader)
