...
zh_to_en_en_token2id, zh_to_en_en_id2token = build_vocabulary(zh_en_train_en_all_tokens)

# Chinese-to-English Chinese id2token and token2id
zh_to_en_zh_token2id, zh_to_en_zh_id2token = build_vocabulary(zh_en_train_zh_all_tokens)

# Vietnamese-to-English English id2token and token2id
vi_to_en_en_token2id, vi_to_en_en_id2token = build_vocabulary(vi_en_train_en_all_tokens)

# Vietnamese-to-English Vietnamese id2token and token2id
vi_to_en_vi_token2id, vi_to_en_vi_id2token = build_vocabulary(vi_en_train_vi_all_tokens)
...
def trainEpoch(train_loader, val_loader, source_id2token, target_id2token, lang_pair, num_epochs = 5, learning_rate = 0.002, decay = 0.8, decay_epoch = 1, bleu_epoch = 1):
    ...
    decoder_optimizer = optim.Adam(rnn_decoder.parameters(), lr=learning_rate)
    criterion = nn.NLLLoss(ignore_index=0, reduction='sum')
    ...
    for epoch in range(num_epochs):
        ...
        # torch.save(encoder.state_dict(), "simple_encoder_batched_" + lang_pair + '_epoch_' + str(epoch)+ ".pth")
        # torch.save(decoder.state_dict(), "simple_decoder_batched_" + lang_pair+ '_epoch_' + str(epoch) + ".pth")
        if epoch % bleu_epoch == 0: 
            print(compute_bleu(encoder, decoder, val_loader, source_id2token, target_id2token, max_length = MAX_LENGTH))

    # torch.save(encoder.state_dict(), "simple_encoder_batched_" + lang_pair  + ".pth")
    # torch.save(decoder.state_dict(), "simple_decoder_batched_" + lang_pair + ".pth")
...
trainEpoch(vi_to_en_train_loader, vi_to_en_val_loader, vi_to_en_vi_id2token, vi_to_en_en_id2token, 'vi_en', num_epochs = 10)
#trainEpoch(some_batches, some_batches, vi_to_en_vi_id2token, vi_to_en_en_id2token, 'vi_en', num_epochs = 80)
